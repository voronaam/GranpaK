...
all_artifacts = ['granpak']

arg_parser = argparse.ArgumentParser('Configure GranpaK')
...
status = subprocess.call("./GRANPAK-VERSION-GEN")
if status != 0:
    print('Version file generation failed')
    sys.exit(1)

file = open('build/GRANPAK-VERSION-FILE', 'r')
granpak_version = file.read().strip()
file = open('build/GRANPAK-RELEASE-FILE', 'r')
granpak_release = file.read().strip()

extra_cxxflags["release.cc"] = "-DGRANPAK_VERSION=\"\\\"" + granpak_version + "\\\"\" -DGRANPAK_RELEASE=\"\\\"" + granpak_release + "\\\"\""
